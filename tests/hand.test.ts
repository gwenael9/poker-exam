...
describe("Two Pair", () => {
  it("should detect two pair", () => {
    const cards = [
      new Card("A", "spades"), new Card("A", "hearts"),
      new Card("K", "diamonds"), new Card("K", "clubs"),
      new Card("9", "spades"), new Card("5", "hearts"),
      new Card("3", "diamonds"),
    ];
    const result = evaluateHand(cards);
  ...
  it("chosen5 should be: higher pair, lower pair, then best kicker", () => {
    ...
    const result = evaluateHand(cards);
    expect(result.chosen5.map(c => c.rank)).toEqual(["A", "A", "K", "K", "9"]);
  });
  ...
  it("should pick the best two pairs when 3 pairs are available", () => {
    const cards = [
      new Card("A", "spades"), new Card("A", "hearts"),
      new Card("K", "diamonds"), new Card("K", "clubs"),
      new Card("9", "spades"), new Card("9", "hearts"),
      new Card("3", "diamonds"),
    ];
    const result = evaluateHand(cards);
    expect(result.chosen5.map(c => c.rank)).toEqual(["A", "A", "K", "K", "9"]);
